...
def handle_query(query, ts1_db):
    ...
    if domain in ts1_db:
        return f"1 {domain} {ts1_db[domain]} {identification} aa"
    else:
...
def main():
    ...
    ts1_db = load_database("ts1database.txt")

    # for domain, ip in ts1_db.items():
    ...
    with open('ts1responses.txt', 'w') as resolved_log:
        while True:
            clientSocket, addr = serverSocket.accept()
            query = clientSocket.recv(1024).decode()
            response = handle_query(query, ts1_db)
